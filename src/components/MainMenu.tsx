...
export const MainMenu = () => {
  ...
  const [clans, setClans] = useState(['iron-crane','shadow-fox','crimson-bear','storm-dragon','jade-turtle']);
  const [url, setUrl] = useState('ws://localhost:3001');
  const [oName, setOName] = useState('');
  const [oClan, setOClan] = useState('iron-crane');
  ...
  return (
    <div className="main-menu">
      ...
      {mode === 'hotseat' && <div className="setup-panel"><h2>Hotseat Setup</h2>
        <div className="player-count-select"><label>Players:</label><select value={pc} onChange={e => setPc(+e.target.value)}>{[2,3,4,5].map(n=><option key={n} value={n}>{n}</option>)}</select></div>
      ...
      <div className="menu-footer"><p>Clans:</p><div className="clan-preview-list">{CLANS.map(c=><div key={c.id} className="clan-preview" style={{borderColor:c.color}}><span className="clan-name" style={{color:c.color}}>{c.name}</span><span className="clan-ability">{c.abilityDescription}</span></div>)}</div></div>
    </div>
  );
};
