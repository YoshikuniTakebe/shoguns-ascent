...
import { CLANS, REGIONS_DATA } from '../types/game';
...
export const GameBoard = () => {
  ...
  const sc: Record<string,string> = { spring:'#FFB7C5', summer:'#FF6B35', autumn:'#D4A574' };
  return (
    <div className="game-board">
      <div className="game-header">
        <div className="season-indicator" style={{backgroundColor:sc[gameState.currentSeason]}}><span className="season-name">{gameState.currentSeason.toUpperCase()}</span><span className="phase-name">{gameState.currentPhase.toUpperCase()} PHASE</span></div>
        <div className="turn-indicator"><span className="current-player-name" style={{color:CLANS.find(c=>c.id===cp?.clanId)?.color}}>{cp?.name}'s Turn</span>{gameState.mode==='hotseat'&&<span className="hotseat-label">[HOTSEAT]</span>}</div>
        <div className="mandate-counter">Mandates: {gameState.politicsMandateCount}/{gameState.maxMandates}</div>
      </div>
      <div className="game-content">
        ...
        <div className="right-panel">{isMyTurn&&gameState.activeBattles.some(b=>!b.resolved)?<BattlePanel/>:<ActionPanel/>}<GameLog/></div>
      </div>
    </div>
  );
};
